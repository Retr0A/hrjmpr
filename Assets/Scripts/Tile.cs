...
public class Tile : MonoBehaviour
{
    ...
    {
        if (collision.gameObject.name == Camera.current.gameObject.transform.parent.gameObject.name)
        {
            if (type == TileType.Water)
            {
                collision.gameObject.GetComponent<PlayerCharacter>().walkingSpeed -= 7.5f;
            }
        }
    }
}
